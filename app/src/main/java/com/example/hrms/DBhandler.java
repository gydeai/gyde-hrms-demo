...
public class DBhandler extends SQLiteOpenHelper {
    ...
    Candidate returnCandidate(int employee_id) {
        ...
        if (cursor != null) {
            cursor.moveToFirst();
            candidate.setName(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_NAME)));
            candidate.setPhone(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_PHONE)));
            candidate.setStatus(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_STATUS)));
            candidate.setPosition(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_POSITION)));
            candidate.setPhotoURI(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_PHOTO)));
            candidate.setResumeURI(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_RESUME)));
            cursor.close();
        }
        return candidate;
    }
    ...
    ArrayList<Candidate> returnCandidates(int number) {
        ...
        if (cursor != null) {
            ...
            while (!cursor.isAfterLast()) {
                Candidate candidate = new Candidate();
                candidate.setId(cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_ID)));
                candidate.setName(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_NAME)));
                candidate.setPhone(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_PHONE)));
                candidate.setStatus(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_STATUS)));
                candidate.setPosition(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_POSITION)));
